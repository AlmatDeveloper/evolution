...
// отрабатывает в самом начале, когда есть только BeanDefinition
...
// PostConstruct проверяется рекурсивно во всех родительских классах
// если аннотация будет проставлена на родительский класс, но при этом этот класс не bean, то метод все равно сработает

// внедрение в самого себя аннотация - Resource

// beanDefinition - ы созданные через java config, не имеют информации о классе
// так как методы не выполняются, не понятно что методы вернут

// Spring дает возможность переопределять bean (через xml, аннотации, java config) приоритет на xml
// в xml имя bean генерируется с его пакетами, в аннотациях имя класса, в config имя метода

// Аннотации в интерфейсах не работают

// несколько Qualifier работают как and оператор