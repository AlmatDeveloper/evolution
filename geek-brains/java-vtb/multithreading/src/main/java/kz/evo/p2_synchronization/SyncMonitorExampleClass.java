...
public class SyncMonitorExampleClass {
    // монитором может служить любой объект
    ...
    // то используем синхронизацию по кастомному монитору
    public void method() {
        for (int i = 0; i < 10; i++) {
        ...
        // когда все потоки дойдут до данной части когда,
        // окажется что только один может выполнить этот блок
        ...
        block: synchronized (monitor) {
            ...
            // то поток который попал в этот блок уже не сможет выйти
            // и будет ждать завершения другого потока
